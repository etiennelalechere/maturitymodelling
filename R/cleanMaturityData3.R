#' @title Clean database including with terrain data selection
#' @description Correct plot names, remove duplicated plots and select defined predictor variables
#' @usage cleanMaturityData3(terrain,metrics,terrain.selec,var.selec,var.names)
#' @param terrain data frame or matrix
#' @param metrics data frame or matrix
#' @param terrain.selec character, names of the terrain variables to keep in the database
#' @param var.selec character, names of the structural variables to keep in the database
#' @param var.names character, to rename variables (optional)
#' @return Return maturity database
#' @export
#' @examples # Not run (tables not available in package): maturity.db=cleanMaturityData3(terrain,metrics,terrain.selec,var.selec,var.names)
cleanMaturityData3<-function(terrain,metrics,terrain.selec,var.selec,var.names){

  # Optional naming of structural variables
  ...
  check <- grep("TGB", colnames(terrain))
  if(length(which(is.na(maturity.db[,check]) == T)) >0){print("NA value for maturity attribute")}
  check <- grep("BMD", colnames(terrain))
  if(length(which(is.na(maturity.db[,check]) == T)) >0){print("NA value for maturity attribute")}
  check <- grep("BMS", colnames(terrain))
  if(length(which(is.na(maturity.db[,check]) == T)) >0){print("NA value for maturity attribute")}
